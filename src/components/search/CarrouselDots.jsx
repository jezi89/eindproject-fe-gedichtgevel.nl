...
export const CarouselDots = memo(({totalCount, currentIndex, onNavigate, hideSeriesNavigation = false, hideRangeIndicator = false}) => {
  ...
  useEffect(() => {
    if (showDropdown && dropdownRef.current && dotsContainerRef.current) {
      const seriesButton = dotsContainerRef.current.querySelector(
        `.${styles.seriesButton}`
      );
  ...
  // Early return AFTER all hooks to prevent React hooks error
  ...
  const handleDotClick = (dotIndex, isWrappedDot = false) => {
    if (onNavigate) {
      ...
      if (isWrappedDot) {
        // For wrapped dots: directly to the real index (0, 1, 2, etc.)
        targetIndex = dotIndex;
      } else {
        // For normal dots: use the existing logic
  ...
  // Format range text for wrap-around
  const formatRangeText = () => {
    if (config.currentRange.wrappedEnd !== null) {
      // Wrap-around case: show all 3 numbers
              ...
              return (
                <button
                  ...
                  className={`${styles.dot} ${
                    element.active ? styles.dotActive : ""
                  } ${element.isPlaceholder ? styles.dotPlaceholder : ""}`}
                  onClick={() => {
                    if (!element.isPlaceholder) {
                      ...
                      if (isWrappedDot) {
                        // For wrapped dots: directly to index (0, 1, 2)
                        handleDotClick(dotIndex, true);
                      } else if (isNextDot) {
                        // For next decade dots: calculate real poem index
                        ...
                        // For normal dots within current decade
                        handleDotClick(dotIndex, false);
                      }
                    }
                  }}
                  aria-label={`Gedicht ${
                    element.poemNumber || element.key.split("-")[1]
                  }`}
              ...
              return (
                <span
                  ...
                  className={`${styles.roman} ${
                    element.isDecadeIndicator ? styles.romanDecade : ""
                  }`}
                  title={
                    element.isDecadeIndicator
                      ? `Tiental ${element.numeral}`
                      : null
                  }
                >
                  {element.numeral}
              ...
              return (
                <span
                  ...
                  className={`${styles.separator} ${
                    element.isDecadeSeparator ? styles.decadeSeparator : ""
                  }`}
                >
                  {element.content}
                </span>
        ...
        {!hideSeriesNavigation && availableDecades.length > 1 && (
          <button
            className={`${styles.seriesButton} ${
              showDropdown ? styles.seriesButtonActive : ""
            }`}
          ...
          >
            ...
            <span
              className={`${styles.seriesArrow} ${
                showDropdown ? styles.seriesArrowUp : ""
              }`}
            >
              ▼
            </span>
      ...
      {!hideRangeIndicator && (
        <span
          className={`${styles.rangeIndicator} ${
            isAudioPage ? styles.rangeIndicatorDark : ""
          }`}
        >
