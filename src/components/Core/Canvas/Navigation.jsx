...
export default function Navigation({
  ...
  const { shareDesign, isSharing, shareError, shareUrl } =
    useCanvasShare(exportUtils);
  ...
  const areSlidersEffectivelyVisible =
    (moveMode === "poem" || moveMode === "line") && xySlidersVisible;
  ...
  const selectionInfo = () => {
    ...
    const poemLineCount = Array.from(selectedLines).filter(
      (idx) => idx >= 0
    ).length;
    ...
    if (hasTitle && hasAuthor && poemLineCount > 0) {
      return {
        ...
        description: `All (${poemLineCount} lines + title + author)`,
      };
    } else if (hasTitle && poemLineCount > 0) {
      return {
        type: "lines-title",
        count: selectedLines.size,
        description: `${poemLineCount} lines + title`,
      };
    } else if (hasAuthor && poemLineCount > 0) {
      return {
        type: "lines-author",
        count: selectedLines.size,
        description: `${poemLineCount} lines + author`,
      };
    } else if (poemLineCount > 0) {
      return {
        type: "lines-only",
        count: poemLineCount,
        description: `${poemLineCount} poem lines`,
      };
    ...
    return {
      type: "other",
      count: selectedLines.size,
      description: `${selectedLines.size} items`,
    };
  };
  ...
  return (
    <div className={styles.navContainer}>
      ...
      <div className={styles.selectionIndicator}>
        ...
        <div className={`${styles.selectionStatus} ${styles[selection.type]}`}>
        ...
        <div className={styles.saveSection}>
          ...
          {getExportDataUrl && (
            <div className={styles.shareSection}>
              ...
              {/* Error display */}
              {shareError && (
                <div className={styles.shareError}>⚠️ {shareError}</div>
              )}
            </div>
          )}
        </div>
