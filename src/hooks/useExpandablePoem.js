...
export const useExpandablePoem = (
  ...
  const updatePoemState = useCallback(
    (updates) => {
      // Poem state update
  ...
  const scrollToContent = useCallback(() => {
    ...
    // Probeer het poemContent element te vinden
    // We gebruiken een class selector op basis van de styles module
    // Als dat niet lukt, vallen we terug op de card zelf
    const contentElement = styles.poemContent
      ? cardRef.current.querySelector(`.${styles.poemContent}`)
      : null;

    const targetElement = contentElement || cardRef.current;

    if (targetElement) {
      const rect = targetElement.getBoundingClientRect();
      const scrollTop =
        window.pageYOffset || document.documentElement.scrollTop;
      // Offset voor header/padding (zodat het niet strak tegen de bovenkant zit)
      const offset = 100;
  ...
  const expandPoem = useCallback(async () => {
    ...
    if (isSmallPoemValue) {
      ...
      // Voor kleine gedichten: direct scrollen
      scrollToContent();
      await new Promise((resolve) => setTimeout(resolve, 100));
      updatePoemState({ phase: "complete" });
    } else {
      updatePoemState({ phase: "expanding", expanded: true });
      // Wacht op de 'spring shift' / layout animatie
      await new Promise((resolve) => setTimeout(resolve, 600));

      // Scrollen voordat de content onthuld wordt
      scrollToContent();
    ...
    updatePoemState,
    index,
    expandablePreview.isExpandable,
    poem,
    scrollToContent,
  ]);

  const collapsePoem = useCallback(
    async (skipScroll = false) => {
