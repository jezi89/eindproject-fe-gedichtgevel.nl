...
export function useCanvasHandlers({ canvasState, currentPoem: poemData = null }) {
  ...
  // Line-specific Text Align Handler
  ...
  const handleSyncAllColorsToGlobal = useCallback(() => {
    ...
    if (
      !confirm(
        `Reset ${totalOverrides} color override${
          totalOverrides === 1 ? "" : "s"
        }?`
      )
    ) {
  ...
  const handleSyncAllFontsToGlobal = useCallback(() => {
    ...
    if (
      !confirm(
        `Reset ${lineFontOverrides} font override${
  ...
  const handleFontFamilyChange = useCallback(
    (newFontFamily) => {
      ...
      if (!availableWeights.includes(currentWeight)) {
        const newWeight = availableWeights.includes("400")
          ? "400"
          : availableWeights[0];
  ...
  return {
    ...
    handleLineFontSizeChange, // fontSize handler
    ...
    handleSkewXChange,
    handleSkewYChange,
    handleLineSkewXChange,
    handleLineSkewYChange,
    handleLineTextAlignChange,

    handleSearchBackground,
    handlePremiumSearch, // Export premium search handler
    handleSetBackground,
